...
def create_circle_kernel(radius, size=None):
    ...
    if size is None:
        size = 2 * radius + 5  # Ensure kernel is large enough
...
class CircleFinderCNN(nn.Module):
    def __init__(self):
        ...
        for radius in [5, 10, 15, 20]:  # Representative radii
            kernel = torch.tensor(create_circle_kernel(radius, size=45), dtype=torch.float32)
            conv = nn.Conv2d(1, 1, kernel_size=kernel.shape[0], padding=kernel.shape[0]//2)
            # Initialize with our circle kernel but allow training
    ...
    def forward(self, x):
        ...
        # Circle kernel path
        circle_features = [F.relu(kernel(x)) for kernel in self.circle_kernels]
        circle_features = torch.cat(circle_features, dim=1)
        circle_features = self.pool(circle_features)
